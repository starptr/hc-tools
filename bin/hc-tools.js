...
app.event("reaction_added", async ({
  ...
  const {
    ...
    channel,
    ts
  } = item; // Ignore blacklisted user

  if (blacklist.includes(user)) return; //List of meaningul reactions
  ...
  try {
    ...
    const [msg, author, reactor] = await Promise.all([app.client.conversations.history({
      token: process.env.SLACK_BOT_TOKEN,
      channel,
      latest: ts,
      limit: 1,
    ...
    switch (reaction) {
      case reactions[0]:
        ...
        if (legitimateReactors.length === 1) {
          ...
          }).catch(err => fancyLog(new Error(`${err.message}. User @ ${reactor.profile.display_name || reactor.profile.real_name} tries to add tool ${ts} in ${channel}, but failed.`)));
        } else {
          fancyLog(`User @ ${reactor.profile.display_name || reactor.profile.real_name} tries to add tool ${ts} in ${channel}, but isn't the first one to add this message so is rejected.`);
        }
      ...
      case reactions[1]:
        ...
        }).catch(err => fancyLog(new Error(`${err.message}. User @ ${reactor.profile.display_name || reactor.profile.real_name} tried to delete message ${ts}, but failed.`)));
        break;
    }
  } catch (err) {
    fancyLog(err);
